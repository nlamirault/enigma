...
docker build -t enigma/release .

echo -e "\033[32;01m[$APP] Make binaries \033[0m"
docker run --rm \
       -v `pwd`:/tmp/ \
       enigma/release \
       gox "${OS_PLATFORM_ARG[@]}" "${OS_ARCH_ARG[@]}" -output="/tmp/enigma_{{.OS}}-{{.Arch}}"

echo -e "\033[32;01m[$APP] Make release \033[0m"
docker run --rm -e GITHUB_TOKEN enigma/release \
...
for BINARY in enigma_*; do
    docker run --rm -e GITHUB_TOKEN -v `pwd`:/go/src/github.com/nlamirault/enigma \
       enigma/release github-release upload \
       --user $USERNAME \
       --repo $REPO \
       --tag $VERSION \
       --name $BINARY \
